...
import type { SceneBackgroundProps } from "./SceneBackground.types";

const easeOutQuint = [0.16, 1, 0.3, 1] as const;
const DEFAULT_TEXTURE = "/textures/watercolor-soft.png";
const DEFAULT_CONFETTI_PALETTE = [
  "#ffbe0b",
  "#fb5607",
  "#ff006e",
  "#8338ec",
...
function ConfettiDots({ palette }: { palette: readonly string[] }) {
  const dots = useMemo(() => {
...
function DecorationsLayer({ scene }: SceneBackgroundProps) {
...
function BalloonClustersLayer({ scene }: SceneBackgroundProps) {
...
function AmbientBalloonsLayer({ scene }: SceneBackgroundProps) {
...
function SceneBackgroundComponent({
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      {showAmbientBalloons ? <AmbientBalloonsLayer scene={scene} /> : null}
    </div>
  );
}
